...
namespace httplibuv
{
	...
	void HttpRequestCall::send(Request& request)
	{
		...
		{
		   debug("Filling host");
		   std::pair<std::string,std::string> hosth("Host", request.method->host);
		...
		int port = request.method->port;
		std::string host(request.method->host);
		void* request_data = (void*) this;

		std::string tmp(request.to_string() + "\r\n");
